...
function CoolButton({text}) {
  ...
  return (
    <motion.button
      className="relative overflow-hidden sm:mt-5 px-4 py-3 text-sm font-semibold text-white rounded-sm"
      style={{
...
function Content() {
  ...
  const button_text = "Kontakt Aufnehmen!"
     const Jsx = () =>{
      return (
      <div className='flex flex-col w-[96%] sm:w-[90%] lg:flex-row justify-center gap-[2rem]'>
        <div className='flex flex-col justify-start content-start lg:p-0 pb-6 lg:h-[800px] min-h[300px] sm:max-h-[1200px] min-w-[300px] max-w-[800px] w-[100%] lg:w-[46%]'>
          ...
          {main_text.map((i, index)=>(<p key={index} className='font-light sm:text-sm text-sm lg:text-[19px] xl:text-[25px] leading-snug py-2 text-green-800'>{i}</p>))}
          <CoolButton text={button_text}/>
        </div>
        <div className='w-[100%] lg:w-[46%] min-w-[300px] max-w-[800px] relative min-h-[300px] max-h-[1000px] h-[700px]'>
          <div className='absolute w-[100%] h-[100%] bg-green-800 -top-[20px] -right-[5px] sm:-top-[40px]  sm:-right-[40px] -z-10'>
          </div>
  ...
  return (
    <div className="relative min-h-screen h-screen max-h-[150vh] flex flex-col items-center gap-20 py-4 lg:py-20">
